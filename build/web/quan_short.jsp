			...
			<div class="row">
				...
				<div class="col-md-12 col-sm-12 hidden-xs sap_xep">
					...
					<select id="sap_xep" onChange="window.location.href=this.value">
                                            <option value="quan_short.jsp?sortKey=bestseller" <% if(sortKey.equals("bestseller")) out.print("selected"); %>>Bán nhiều nhất</option>
                                            <option value="quan_short.jsp?sortKey=newest" <% if(sortKey.equals("newest")) out.print("selected"); %>>Mới nhất</option>
                                            <option value="quan_short.jsp?sortKey=maxtomin" <% if(sortKey.equals("maxtomin")) out.print("selected"); %>>Giá: Từ cao đến thấp</option>
                                            <option value="quan_short.jsp?sortKey=mintomax" <% if(sortKey.equals("mintomax")) out.print("selected"); %>>Giá: Từ thấp đến cao</option>
                                            <option value="quan_short.jsp?sortKey=atoz" <% if(sortKey.equals("atoz")) out.print("selected"); %>>A-Z</option>
                                            <option value="quan_short.jsp?sortKey=ztoa" <% if(sortKey.equals("ztoa")) out.print("selected"); %>>Z-A</option>
					</select>
				</div>
