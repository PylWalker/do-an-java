...
<%
    ...
    list = sanPhamGet.getAoVest(sortKey);
%>
<body>
			...
			<div class="row">
				...
				<div class="col-md-12 col-sm-12 hidden-xs sap_xep">
					...
					<select id="sap_xep" onChange="window.location.href=this.value">
                                            <option value="ao_vest.jsp?sortKey=bestseller" <% if(sortKey.equals("bestseller")) out.print("selected"); %>>Bán nhiều nhất</option>
                                            <option value="ao_vest.jsp?sortKey=newest" <% if(sortKey.equals("newest")) out.print("selected"); %>>Mới nhất</option>
                                            <option value="ao_vest.jsp?sortKey=maxtomin" <% if(sortKey.equals("maxtomin")) out.print("selected"); %>>Giá: Từ cao đến thấp</option>
                                            <option value="ao_vest.jsp?sortKey=mintomax" <% if(sortKey.equals("mintomax")) out.print("selected"); %>>Giá: Từ thấp đến cao</option>
                                            <option value="ao_vest.jsp?sortKey=atoz" <% if(sortKey.equals("atoz")) out.print("selected"); %>>A-Z</option>
                                            <option value="ao_vest.jsp?sortKey=ztoa" <% if(sortKey.equals("ztoa")) out.print("selected"); %>>Z-A</option>
					</select>
				</div>
